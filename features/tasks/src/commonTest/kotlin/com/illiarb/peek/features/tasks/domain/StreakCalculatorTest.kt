...
internal class StreakCalculatorTest {
  ...
  fun `it should return 1 when all habits completed only today`() {
    ...
    val habit = HabitInfo(
      id = "h1",
      createdAt = LocalDate(2026, 1, 10).atTime(0, 0).toInstant(TimeZone.UTC),
    )
  ...
  fun `it should ignore days before habit creation`() {
    ...
    val habit = HabitInfo(
      id = "h1",
      createdAt = LocalDate(2026, 1, 13).atTime(0, 0).toInstant(TimeZone.UTC),
    )
    val completions = listOf(
      TaskCompletion("h1", today),
      TaskCompletion("h1", LocalDate(2026, 1, 14)),
  ...
  fun `it should handle long streaks correctly`() {
    ...
    val habit = HabitInfo(
      id = "h1",
      createdAt = LocalDate(2026, 1, 1).atTime(0, 0).toInstant(TimeZone.UTC),
    )
